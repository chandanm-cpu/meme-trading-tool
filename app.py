...
def train_rf(df):
    ...
    y = df["label_outcome"]
    m = RandomForestClassifier(n_estimators=300, max_depth=10, random_state=42)
    m.fit(X,y)
    return m
...
def survival_from_probs(probs, hours):
    rug_prob = sum(v for k,v in probs.items() if "RUG" in k)
    decay = 0.85 if hours == 72 else 1.0
    return int(max(0, (1 - rug_prob * decay) * 100))
